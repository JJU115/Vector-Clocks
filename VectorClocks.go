/*
	...
	the event's number and display the updated timestamp with the corresponding event number.

	Ex.
	(0) Internal event generated by process 3
	Process 3 event 0 timestamp: [0 0 0 1 0]
...
func process(num int) {
	...
	for i := 0; i < rand.Intn(10); i++ {
		...
		if rand.Intn(50)%2 == 0 { //Send an altered array to the clock holder to signify an internal process event
			fmt.Printf("(%d) Internal event generated by process %d\n", eNum, num)
			...
			fmt.Printf("(%d) Process %d sending message to process %d\n", eNum, num, dest)
			sendChan <- dest
		}
		eNum++
	}
